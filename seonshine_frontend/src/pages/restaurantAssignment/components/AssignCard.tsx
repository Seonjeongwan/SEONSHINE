...
type AssignCardProps = {
  assignData: AssignTableType;
  handleSelectChange: (id: number, userId: string) => void;
  isSelectDisabled: (id: number) => boolean;
};

const AssignCard = ({ assignData, handleSelectChange, isSelectDisabled }: AssignCardProps) => {
  const { address, assignedDate, restaurants, selectedRestaurantId } = assignData;
  return (
    <Stack
      direction="column"
      gap={2}
